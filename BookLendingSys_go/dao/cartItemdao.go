...
func AddCartItem(cartItem *model.CartItem_chenjunjie) error {
	//写sql
...
func GetCartItemByBookIDAndCartID(bookID string, cartID string) (*model.CartItem_chenjunjie, error) {
	//写sql语句
	...
	//设置一个变量接收图书的id
	//创建cartItem
	cartItem := &model.CartItem_chenjunjie{}
...
func UpdateBookCount(cartItem *model.CartItem_chenjunjie) error {
	//写sql语句
...
func GetCartItemsByCartID(cartID string) ([]*model.CartItem_chenjunjie, error) {
	//写sql语句
	...
	var cartItems []*model.CartItem_chenjunjie
	for rows.Next() {
		//设置一个变量接收bookId
		var bookID string
		//创建cartItem
