...
func GetBooks() ([]*model.Book_chenjunjie, error) {
	//写sql语句
	...
	var books []*model.Book_chenjunjie
	for rows.Next() {
...
func GetPageBooks(pageNo string) (*model.Page_chenjunjie, error) {
	//将页码转换为int64类型
...
func GetPageBooksByPrice(pageNo string, minPrice string, maxPrice string) (*model.Page_chenjunjie, error) {
	//将页码转换为int64类型
	iPageNo, _ := strconv.ParseInt(pageNo, 10, 64)
	//获取数据库中图书的总记录数
